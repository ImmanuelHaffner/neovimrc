...
local function parse_rule_file(file_path)
    ...
    if not ok then
        vim.notify('Failed to read rule file: ' .. file_path, vim.log.levels.WARN)
        return {}, {}
    end
    local i = 1
    local header = {}
    ...
    while i <= #lines and utils.trim(lines[i]) == '' do
        i = i + 1
    end
...
return {
    ...
    { 'olimorris/codecompanion.nvim',
        ...
        config = function()
            ...
            --         -- Build new prompt library entries in a temporary table to avoid
            --         -- modifying cc_config.prompt_library while it might be in use.
            --         local new_prompts = {}
            --
            ...
            --
            --         -- Apply all changes atomically via vim.schedule to avoid race conditions
            --         -- with CodeCompanion potentially iterating over prompt_library.
            --         vim.schedule(function()
            --             -- Merge new prompts into the prompt library
            --             for name, prompt in pairs(new_prompts) do
            --                 cc_config.prompt_library[name] = prompt
            --             end
            --
